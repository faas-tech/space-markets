...
contract VerifyDeployment is Script {
    ...
    function _verifyPermissions(ContractInfo memory info) internal view {
        ...
        // AssetRegistry roles
        _check("AssetRegistry DEFAULT_ADMIN_ROLE",
            assetRegistry.hasRole(assetRegistry.DEFAULT_ADMIN_ROLE(), info.admin));
        _check("AssetRegistry ADMIN_ROLE",
            assetRegistry.hasRole(assetRegistry.ADMIN_ROLE(), info.admin));
        _check("AssetRegistry REGISTRAR_ROLE",
            assetRegistry.hasRole(assetRegistry.REGISTRAR_ROLE(), info.admin));

        // LeaseFactory roles
        _check("LeaseFactory DEFAULT_ADMIN_ROLE",
            leaseFactory.hasRole(leaseFactory.DEFAULT_ADMIN_ROLE(), info.admin));
        _check("LeaseFactory ADMIN_ROLE",
            leaseFactory.hasRole(leaseFactory.ADMIN_ROLE(), info.admin));

        // Marketplace roles
        _check("Marketplace DEFAULT_ADMIN_ROLE",
            marketplace.hasRole(marketplace.DEFAULT_ADMIN_ROLE(), info.admin));
        _check("Marketplace ADMIN_ROLE",
            marketplace.hasRole(marketplace.ADMIN_ROLE(), info.admin));
    ...
    function _verifyInterconnections() internal view {
        ...
        _check("LeaseFactory -> AssetRegistry connection",
            address(leaseFactory.registry()) == address(assetRegistry));
        _check("Marketplace -> LeaseFactory connection",
            address(marketplace.leaseFactory()) == address(leaseFactory));
        _check("Marketplace -> Stablecoin connection",
            address(marketplace.stable()) == address(stablecoin));
