...
//                 abi.encodePacked(type(MockStablecoin).creationCode, abi.encode("Test USDC", "USDC", 6));
...
//         require(
//             AssetRegistry(assetRegistry).hasRole(AssetRegistry(assetRegistry).DEFAULT_ADMIN_ROLE(), admin),
//             "AssetRegistry admin role not set"
//         );
//         require(AssetRegistry(assetRegistry).nextTypeId() == 1, "AssetRegistry not properly initialized");

//         // Verify LeaseFactory
//         require(
//             LeaseFactory(leaseFactory).hasRole(LeaseFactory(leaseFactory).DEFAULT_ADMIN_ROLE(), admin),
//             "LeaseFactory admin role not set"
//         );
//         require(
//             address(LeaseFactory(leaseFactory).registry()) == assetRegistry, "LeaseFactory registry not set correctly"
//         );

//         // Verify Marketplace
//         require(
//             Marketplace(marketplace).hasRole(Marketplace(marketplace).DEFAULT_ADMIN_ROLE(), admin),
//             "Marketplace admin role not set"
//         );
//         require(address(Marketplace(marketplace).stable()) == stablecoin, "Marketplace stablecoin not set correctly");
//         require(
//             address(Marketplace(marketplace).leaseFactory()) == leaseFactory,
//             "Marketplace lease factory not set correctly"
//         );
...
//                 '  "contracts": {\n',
//                 '    "AssetRegistry": "',
//                 vm.toString(assetRegistry),
//                 '",\n',
//                 '    "LeaseFactory": "',
//                 vm.toString(leaseFactory),
//                 '",\n',
//                 '    "Marketplace": "',
...
//             string(abi.encodePacked("./deployments/", vm.toString(block.chainid), "-", config.networkName, ".json"));
